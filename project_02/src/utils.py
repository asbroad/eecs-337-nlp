...
def prettyPrintIngredients(ingredient_list):
	for item in ingredient_list:
		line = "Name: {}\nType: {}\nQty: {}\nMeasure: {}\nPrep: {}\nPrep Description: {}\n".format(item.name, item.description, item.qty, item.measure, item.prep, item.prep_description)
		print(line)
...
def getIngredients(page):
	...
	for entry in ingredients:
		...
		res = findDescriptor(ingredient_name)
		ingredient_name = res[0]
		ingredient_description = res[1]
		...
		item.ingredient_from_recipie(ingredient_name, ingredient_qty, ingredient_measure, ingredient_prep, ingredient_prep_description, ingredient_description)
...
def hc_make_method_list():
	...
	methods = ["Grill", "grilling","Stir-Fry", "Flambe", "Fry", "Pan-Fry", "Deep Fry", "Saute"]
	methods.extend(["Boil", "Roast", "Bake", "Sear", "Poach", "Simmer", "Broil", "Steam", "Blanch", "Braise"])
	methods.extend(["Stew", "grease", "greasing", "preheat", "preheating", "mix", "mixing", "melted", "melting", "arrange"])
	methods.extend(["arranging", "microwave", "microwaving", "coat", "coating", "basting", "broil", "broiling", "turning"])
	methods.extend(["sprinkle", "sprinkling", "whisk", "whisking", "sear", "searing", "spread", "spreading"])
	methods.extend(["mash", "mashing", "simmer"])
	return [method.lower() for method in methods]


def unabridgeMeasure(recipe):
	#ingredient measures from "convert-me.com/en/convert/cooking"
	unabridged_ingredients = []
	...
	recipe.ingredients = unabridged_ingredients
	return recipe


def findDescriptor(ingredient_name):
	name = ingredient_name.lower()
	ingredient_description_list = []
	descriptor_list = ["fresh", "raw", "organic", "natural", "pure", "cooked", "dried", "extra-virgin", "whole-wheat", "gluten-free", "fat-free", "low-sodium", "boneless"]
	for descriptor in descriptor_list:
		if descriptor in name:
			name = name.replace(descriptor, "").strip()
			ingredient_description_list.append(descriptor)

		elif descriptor.replace("-", " ") in name:
			name = name.replace(descriptor.replace("-", " "), "").strip()
			ingredient_description_list.append(descriptor)

	name = name.replace("  ", " ")
	return [name, ingredient_description_list]
